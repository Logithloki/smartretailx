...
export function CartPage() {
  ...
  if (confirmed) return <section className="form-card"><h1>Order confirmed</h1><p>Order <code>{confirmed.orderId}</code> was accepted for stock reservation.</p><div className="summary-row"><span>Subtotal</span><strong>{formatCurrency(confirmed.subtotal)}</strong></div><div className="summary-row"><span>Discount</span><strong>−{formatCurrency(confirmed.discountTotal)}</strong></div><div className="summary-row total"><span>Authoritative total</span><strong>{formatCurrency(confirmed.totalAmount)}</strong></div><Link className="btn" to="/orders">Track delivery</Link></section>;
